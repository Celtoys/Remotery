...
TitleWindow = (function()
{
	function TitleWindow(wm, settings, server, connection_address)
	{
		...
		this.PingContainer = this.Window.AddControl(new WM.Container(4, -13, 10, 10));
		DOM.Node.AddClass(this.PingContainer.Node, "PingContainer");

		this.EditBox = this.Window.AddControl(new WM.EditBox(10, 5, 300, 18, "Connection Address", connection_address));

		// Setup pause button
		this.PauseButton = this.Window.AddControl(new WM.Button("Pause", 5, 5, { toggle: true }));
	...
	TitleWindow.prototype.WindowResized = function(width, height)
	{
		...
		this.PauseButton.SetPosition(width - 80, 7);
	}
